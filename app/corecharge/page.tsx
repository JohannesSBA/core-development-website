...
function GooglePlayIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="w-6 h-6 shrink-0">
      <path d="M3.5 2.5C3.2 2.8 3 3.3 3 3.9V20.1C3 20.7 3.2 21.2 3.5 21.5L13.5 11.5L3.5 2.5Z" fill="#00D4FF" />
      <path d="M16.8 8.3L5.3 1.8C4.7 1.5 4.1 1.5 3.6 1.8L13.6 11.5L16.8 8.3Z" fill="#00F076" />
      <path d="M13.6 11.5L3.6 21.2C4.1 21.5 4.7 21.5 5.3 21.2L16.8 14.7L13.6 11.5Z" fill="#FF3A52" />
      <path d="M20.4 9.9L16.8 8.3L13.6 11.5L16.8 14.7L20.4 13.1C21.4 12.6 21.4 10.4 20.4 9.9Z" fill="#FFC700" />
    </svg>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col leading-none">
        <span className="text-[10px] font-normal tracking-widest text-white/60 uppercase">Download on the</span>
        <span className="text-[17px] font-semibold text-white mt-0.5" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>App Store</span>
      </div>
    </a>
  )
}
...
function ComingSoonBadge() {
  return (
    <div
      ...
      className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-2xl px-5 h-14 cursor-default select-none"
    >
      <GooglePlayIcon />
      <div className="flex flex-col leading-none">
        <span className="text-[10px] font-normal tracking-widest text-white/40 uppercase">Coming soon on</span>
        <span className="text-[17px] font-semibold text-white/50 mt-0.5" style={{ fontFamily: "system-ui, -apple-system, sans-serif" }}>Google Play</span>
      </div>
    </div>
  )
}
...
const features: { icon: React.ElementType; label: string }[] = [
  { icon: MapPin,      label: "Real-time station availability and live charger status" },
  { icon: Battery,     label: "Estimated State of Charge for AC and DC charging sessions" },
  { icon: Activity,    label: "Battery health monitoring with scientific degradation analysis" },
  { icon: Smartphone,  label: "iOS Live Activity and Dynamic Island live session tracking" },
  { icon: Users,       label: "Smart queue management with real-time position updates" },
  { icon: Car,         label: "Multi-vehicle profile support for up to 3 vehicles" },
  { icon: UserPlus,    label: "Guest vehicle mode for operators charging customer cars" },
  { icon: FileText,    label: "Instant digital receipts with full session details" },
  { icon: Star,        label: "Loyalty rewards on every charging session" },
  { icon: Zap,         label: "Power outage detection with proximity alerts" },
  { icon: Shield,      label: "Idle fee protection with automatic session safeguards" },
  { icon: Wifi,        label: "Offline-ready with intelligent data caching" },
  { icon: Lock,        label: "Bank-grade security with end-to-end encrypted transactions" },
]
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: "linear-gradient(180deg, #0b3236 0%, #0d3a3e 40%, #0c2f32 100%)",
      ...
      >
        ...
        <section className="relative z-10 mx-auto max-w-6xl px-6 pt-24 pb-28 flex flex-col lg:flex-row items-center gap-16 lg:gap-20">
          ...
          <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-teal-400/10 px-4 py-1.5 mb-8">
              ...
              <span className="text-teal-300 text-xs font-medium tracking-widest uppercase">Now Live on iOS</span>
            </div>
            ...
            <p className="mt-6 text-white/60 text-lg leading-relaxed max-w-lg">
              Find stations, start sessions, and manage your wallet — with faster sign-in and a smoother experience on any connection.
            </p>
        ...
        {/* ── STATS ─────────────────────────────────────────────────────── */}
        <section className="relative z-10" style={{ boxShadow: "0 1px 0 0 rgba(0,0,0,0.4), 0 -1px 0 0 rgba(0,0,0,0.4)" }}>
        ...
        <section className="relative z-10 mx-auto max-w-6xl px-6 py-24">
          <div className="text-center mb-16">
            <h2 className="text-white font-bold text-3xl sm:text-4xl tracking-tight">Find. Charge. Go.</h2>
            <p className="mt-4 text-white/50 max-w-lg mx-auto text-base leading-relaxed">
              Designed from the ground up for the modern African EV driver — reliability, transparency, and precision at every charge.
            </p>
          </div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {highlights.map(({ icon: Icon, title, body }) => (
              <div
                ...
                style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.35), 0 1px 3px rgba(0,0,0,0.25)" }}
              >
        ...
        <section className="relative z-10 mx-auto max-w-6xl px-6 pb-24">
          <div className="rounded-3xl bg-white/3 overflow-hidden" style={{ boxShadow: "0 2px 24px rgba(0,0,0,0.4), 0 1px 4px rgba(0,0,0,0.3)" }}>
            <div className="grid lg:grid-cols-2 gap-0">

              {/* Feature list */}
              <div className="p-10 lg:p-14" style={{ boxShadow: "inset -1px 0 0 rgba(0,0,0,0.3)" }}>
                <h2 className="text-white font-bold text-2xl sm:text-3xl tracking-tight mb-2">
                  Everything you need, nothing you don't
                </h2>
                <p className="text-white/40 text-sm mb-10">
                  Every session fully audited. Every transaction encrypted. Every receipt instant.
                </p>
                ...
                <ul className="space-y-4">
                  {features.map(({ icon: Icon, label }) => (
                    <li key={label} className="flex items-start gap-3.5">
                      ...
                      <span className="text-white/65 text-sm leading-snug">{label}</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <section className="relative z-10 mx-auto max-w-6xl px-6 pb-24">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="rounded-2xl bg-white/4 p-8" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.35), 0 1px 3px rgba(0,0,0,0.25)" }}>
              <div className="w-10 h-10 rounded-xl bg-teal-500/15 flex items-center justify-center mb-6">
                <Building2 className="w-5 h-5 text-teal-400" strokeWidth={1.5} />
              </div>
              <h3 className="text-white font-semibold text-xl mb-3">Operator Ready</h3>
              <p className="text-white/50 text-sm leading-relaxed">
                Operator accounts support guest vehicle profiles, enabling seamless customer charging without saving personal data — ideal for hotels, parking facilities, and commercial charging businesses.
              </p>
            </div>
            ...
            <div className="rounded-2xl bg-white/4 p-8" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.35), 0 1px 3px rgba(0,0,0,0.25)" }}>
              ...
              <h3 className="text-white font-semibold text-xl mb-3">Enterprise-Grade Infrastructure</h3>
              <p className="text-white/50 text-sm leading-relaxed">
                Powered by OCPP 1.6J certified communication, real-time network health monitoring, and automatic session recovery — engineered for 99.9% uptime and built to scale across Ethiopia and beyond.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="relative z-10 pb-28">
          ...
          >
            <div className="mx-auto max-w-2xl text-center rounded-3xl bg-white/4 px-10 py-16" style={{ boxShadow: "0 2px 24px rgba(0,0,0,0.4), 0 1px 4px rgba(0,0,0,0.3)" }}>
              <span className="text-teal-400 text-xs font-semibold tracking-widest uppercase">Get Started</span>
