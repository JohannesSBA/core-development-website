...
const getIconForTitle = (title: string) => {
  ...
  if (titleLower.includes("energy") || titleLower.includes("infrastructure"))
    return <Zap className="h-5 w-5" />;
  if (titleLower.includes("water")) return <Droplets className="h-5 w-5" />;
  if (titleLower.includes("transport") || titleLower.includes("charging"))
    return <Truck className="h-5 w-5" />;
  if (titleLower.includes("health") || titleLower.includes("pharmacy"))
    return <Heart className="h-5 w-5" />;
  if (titleLower.includes("agriculture") || titleLower.includes("farming"))
    return <Leaf className="h-5 w-5" />;
  if (titleLower.includes("microcredit") || titleLower.includes("financial"))
...
export default function OrganizationalHoverCard({
  ...
  return (
    <HoverCard>
      ...
      <HoverCardContent
        className="w-80 lg:w-96 p-0 bg-white shadow-2xl border-0 rounded-2xl overflow-hidden"
        sideOffset={8}
      >
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2 lg:space-x-3">
                <div className="p-1 lg:p-2 bg-white/20 rounded-lg">
                  <div className="h-4 w-4 lg:h-5 lg:w-5">{displayIcon}</div>
                </div>
